...
// Sesuaikan props dengan data yang dikirim dari parent
defineProps<{
  data: any[]
  headers: any[]
  loading?: boolean
    ...
    <AppTable v-else :headers="headers">
      ...
      >
        ...
        <td class="px-6 py-4">
          <div class="flex items-center gap-2">
            <!-- <div
              class="w-7 h-7 bg-blue-600 rounded-full flex items-center justify-center text-[10px] font-bold text-white uppercase shadow-sm"
            >
              {{ (trx.user_name || 'AD').substring(0, 2) }}
            </div> -->
        ...
        <td class="px-6 py-4 text-gray-500">
          ...
          >
            {{ currentTab === 'trash' ? '-' : '+' }} {{ getTotalQty(trx) }}
          </span>
        </td>
      ...
      <tr v-if="data.length === 0">
        <td :colspan="headers.length" class="px-6 py-10 text-center text-gray-400 italic">
          Data riwayat produk masuk tidak ditemukan.
        </td>
      </tr>
    </AppTable>
  </div>
