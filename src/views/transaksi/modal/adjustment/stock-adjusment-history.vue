  ...
  <div class="space-y-4">
    ...
    <AppTable v-else :headers="headers">
      <tr v-for="trx in data" :key="trx.id" class="hover:bg-gray-50 transition-colors text-sm">
        <td
          class="px-6 py-4 font-bold text-gray-500 uppercase tracking-tighter cursor-pointer hover:underline"
        ...
        <td class="px-6 py-4">
          <div class="flex items-center gap-2">
            ...
            <span class="text-gray-700">{{ trx.user_name || 'Unknown' }}</span>
          </div>
        </td>
        ...
        <td
          ...
          :class="trx.jenis === 'MASUK' ? 'text-green-600' : 'text-red-600'"
        >
          {{ trx.jenis === 'MASUK' ? '+' : '-' }}{{ trx.total_qty }}
        </td>
