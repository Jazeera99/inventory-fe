  ...
  <div class="space-y-4">
    <AppTable :headers="headers">
      <tr v-for="trx in data" :key="trx.id" class="hover:bg-gray-50 transition-colors text-sm">
        <td class="px-6 py-4 font-bold uppercase tracking-tighter">
          <button
            @click="$emit('view-detail', trx)"
            class="font-medium text-gray-500 text-blue-600 hover:underline hover:text-blue-800"
          >
            {{ trx.transaction_no }}
          </button>
        </td>
        ...
        <td class="px-6 py-4">
          <div class="flex items-center gap-2">
            ...
            <span class="font-medium text-gray-500">{{ trx.user_name || 'Unknown' }}</span>
          </div>
        </td>
